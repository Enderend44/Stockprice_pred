...
def main():
    ...
    parser.add_argument("--loss_function", type=str, default="mse_loss", choices=["mse_loss", "mae_loss", "huber_loss", "custom_loss", "rmse_loss", "log_cosh_loss", "smooth_l1_loss"], help="Choose the loss function.")
    ...
    # Générer un nom de fichier unique pour le modèle avec les hyperparamètres et la fonction de perte
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    checkpoint_path = os.path.join(
        checkpoint_folder, 
        f"{args.model}_model_{timestamp}_batch{args.batch_size}_seq{args.seq_length}_epochs{args.epochs}_lr{args.lr}_{args.loss_function}.h5"
    )
